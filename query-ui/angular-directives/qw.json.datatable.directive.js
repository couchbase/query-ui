 ...
 *     <div class="wb-results-table" style="height:500px" [qwJsonDataTable2]="item_array"></div>
 *
 * So the <div> is set up with whatever properties are desired. Note the "[]" around the
...
function createTable(json, element, renderer) {
  ...
  if (json) {
    ...
    if (meta) {
      ...
      tableWrapper = angular.element('<div class="data-table-wrapper show-scrollbar"></div>')[0];
      // start with a dummy empty div as big vertically as we need it to be
      tableHTML = '<div class="data-table" style="height: ' +
    ...
    //
      ...
      tableWrapper = angular.element('<div class="data-table-wrapper">Unable to process data, see log.</div>')[0];
      renderer.appendChild(element,tableWrapper);
