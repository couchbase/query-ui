    ...
    function buildPhoneHomeThingy(source) {
      ...
      var stats = {
        ...
        istats: {
          ...
          avg_sync_write_last_week: [], //Average vb_active_sync_write / sec last week
      ...
      _.each(perBucketStats, function (perBucketStat, index) {
        ...
        if (statsInfo) {
          ...
          setPerBucketStat(stats, "avg_sync_write", bucketStats.vb_active_sync_write_committed_count);
          ...
          //console.log("Got stats for " + bucketName + ": " + JSON.stringify(stats,null,2));
        }
      });
      ...
      calculateAvgWeekAndHour(stats, "avg_sync_write", true);
